...
class IncomeTax:
    ...
    def calculate(self):
        employment_income = next(income for income in self._incomes if type(income) is Employment)
        tax_free, basic_rate, higher_rate, additional_rate = self.get_bands(employment_income.get_salary())
        tax = (basic_rate * 0.2) + (higher_rate * 0.4) + (additional_rate * 0.45)
        return tax

    def get_bands(self, income):
        personal_allowance = next(allowance for allowance in self._allowances if type(allowance) is PersonalAllowance)
